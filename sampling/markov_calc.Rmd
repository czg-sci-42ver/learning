...
> from https://github.com/Accio/fiveMinuteStats/tree/fc5fe81e27aa3e1dbfcf4d8a4db69fc8088fa683
...
> also see `inverse_mat.ipynb` in `python branch`
```{r}
r <- eigen(t(P))
V <- r$vectors
V
...
> see `plot.ipynb` in python branch
```{r}
lam[2]
lam[3]
1 / lam[2]
...
> ? t(solve(t(A_constr)%*%A_constr,t(A_constr)%*%b_constr))
